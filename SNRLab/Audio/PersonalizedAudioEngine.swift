...
final class PersonalizedAudioEngine: ObservableObject {
    ...
    func load(
        ...
        load(
            track: track,
            bands: CompensationDesigner.bands(
                from: pureToneTest,
                maximumBoostDB: maximumBoostDB
            ),
            fixedHeadroomMaximumBoostDB: nil
        )
    }
    ...
    /// Applies externally designed left/right bands. A fixed headroom value lets
    /// blinded comparisons keep the same master attenuation across candidates so
    /// that a louder preset is not automatically favored.
    func updateBands(
        _ newBands: [StereoCompensationBand],
        fixedHeadroomMaximumBoostDB: Double?
    ) {
        bands = normalizedBands(newBands)
        ...
        let calculatedMaximum = bands.flatMap { [$0.leftGainDB, $0.rightGainDB] }.max() ?? 0
        let maximumBoost = max(0, fixedHeadroomMaximumBoostDB ?? calculatedMaximum)
    ...
    private func normalizedBands(_ proposed: [StereoCompensationBand]) -> [StereoCompensationBand] {
        CompensationDesigner.frequencies.map { frequency in
            let nearest = proposed.min {
                abs($0.frequency - frequency) < abs($1.frequency - frequency)
            }
            return StereoCompensationBand(
                frequency: frequency,
                leftGainDB: nearest?.leftGainDB ?? 0,
                rightGainDB: nearest?.rightGainDB ?? 0
            )
        }
    }
