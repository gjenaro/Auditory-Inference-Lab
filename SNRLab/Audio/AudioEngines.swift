...
struct AudioOutputRouteStatus: Equatable {
    ...
    let isAirPods: Bool
    let isBoseQuietComfort: Bool
    let isBluetooth: Bool

    var isStereo: Bool { channelCount >= 2 }
    var isSupportedHeadset: Bool { isAirPods || isBoseQuietComfort }
    var isReadyForBilateralTest: Bool { isSupportedHeadset && isStereo }
}
...
final class AudioSessionManager {
    ...
    func outputRouteStatus(activatePlayback: Bool = true) throws -> AudioOutputRouteStatus {
        ...
        let isAirPods = isBluetooth && SupportedHeadsetClassifier.isAirPodsRouteName(name)
        let isBoseQuietComfort = isBluetooth && SupportedHeadsetClassifier.isBoseQuietComfortRouteName(name)
        return AudioOutputRouteStatus(
            name: name,
            channelCount: session.outputNumberOfChannels,
            isAirPods: isAirPods,
            isBoseQuietComfort: isBoseQuietComfort,
...
final class StimulusEngine: ObservableObject {
    ...
    /// Removes periodic portions of the synthesized sentence. In the
    /// noise-filled condition only the removed regions receive filler noise;
    /// no missing speech information is restored to those regions.
    func playInterruptedSpeech(
        _ sentence: String,
        condition: PredictiveCondition,
        language: TestLanguage,
        voiceProfile: VoiceProfile,
        interruptionRate: Double = 3.0,
        speechDutyCycle: Double = 0.62
    ) async {
        stop()
        isPlaying = true
        lastError = nil
        do {
            try AudioSessionManager.shared.configureForPlayback()
            let pieces = try await synthesize(sentence, language: language, profile: voiceProfile)
            guard let speech = concatenate(pieces) else {
                throw NSError(
                    domain: "SNRLab",
                    code: 7,
                    userInfo: [NSLocalizedDescriptionKey: "Unable to prepare interrupted speech."]
                )
            }
            normalize([speech], toActiveRMS: 0.12)
            applyInterruptions(
                to: speech,
                fillWithNoise: condition == .noiseFilledGap,
                rate: interruptionRate,
                dutyCycle: speechDutyCycle
            )
            try play(speechBuffers: [speech], noiseBuffer: nil)
        } catch {
            lastError = error.localizedDescription
    ...
    /// Produces a repeatable exploratory degradation using limited bandwidth,
    /// shallow envelope modulation, and low-level noise. It is intentionally
    /// labeled filtered speech rather than a clinical vocoder simulation.
    func playFilteredSpeech(
        _ sentence: String,
        language: TestLanguage,
        voiceProfile: VoiceProfile
    ) async {
        ...
        do {
            ...
            guard let speech = concatenate(pieces) else {
                throw NSError(
                    domain: "SNRLab",
                    code: 8,
                    userInfo: [NSLocalizedDescriptionKey: "Unable to prepare filtered speech."]
                )
            }
            normalize([speech], toActiveRMS: 0.12)
            applyExploratoryDegradation(to: speech)
            normalize([speech], toActiveRMS: 0.10)
            try play(speechBuffers: [speech], noiseBuffer: nil)
        } catch {
            lastError = error.localizedDescription
            isPlaying = false
    ...
    private func concatenate(_ buffers: [AVAudioPCMBuffer]) -> AVAudioPCMBuffer? {
        ...
        ), let destination = combined.floatChannelData else { return nil }
        ...
        for buffer in buffers {
            guard let source = buffer.floatChannelData,
                  buffer.format.channelCount == first.format.channelCount else { return nil }
            let frames = Int(buffer.frameLength)
            let bytes = frames * MemoryLayout<Float>.size
            for channel in 0..<channelCount {
                memcpy(destination[channel].advanced(by: writeOffset), source[channel], bytes)
            }
            writeOffset += frames
        }
        return combined
    ...
    private func applyInterruptions(
        ...
        let period = max(1, Int((sampleRate / max(0.5, rate)).rounded()))
        let onFrames = min(period - 1, max(1, Int(Double(period) * min(0.9, max(0.3, dutyCycle)))))
        let rampFrames = max(1, Int(sampleRate * 0.008))
        let targetNoiseAmplitude = min(0.24, activeRMS(of: [buffer]) * 1.55)
        ...
        for channel in 0..<Int(buffer.format.channelCount) {
            for frame in 0..<Int(buffer.frameLength) {
                ...
                if phase < onFrames {
                    let distanceFromStart = phase
                    let distanceFromEnd = onFrames - phase - 1
                    let edge = min(1, Double(min(distanceFromStart, distanceFromEnd)) / Double(rampFrames))
                    data[channel][frame] *= Float(max(0, edge))
                } else if fillWithNoise {
                    let gapPosition = phase - onFrames
                    let gapLength = period - onFrames
                    let distanceFromEnd = gapLength - gapPosition - 1
                    let edge = min(1, Double(min(gapPosition, distanceFromEnd)) / Double(rampFrames))
                    let noise = Double.random(in: -1...1, using: &generator) * targetNoiseAmplitude
                    data[channel][frame] = Float(noise * max(0, edge))
                } else {
    ...
    private func applyExploratoryDegradation(to buffer: AVAudioPCMBuffer) {
        guard let data = buffer.floatChannelData else { return }
        let sampleRate = buffer.format.sampleRate
        let pole = exp(-2 * Double.pi * 3_200 / sampleRate)
        var generator = SystemRandomNumberGenerator()

        for channel in 0..<Int(buffer.format.channelCount) {
            var lowPass = 0.0
            for frame in 0..<Int(buffer.frameLength) {
                let input = Double(data[channel][frame])
                lowPass = (1 - pole) * input + pole * lowPass
                let time = Double(frame) / sampleRate
                let modulation = 0.66 + 0.34 * abs(sin(2 * .pi * 3.2 * time))
                let noise = Double.random(in: -1...1, using: &generator) * 0.012
                data[channel][frame] = Float(min(0.90, max(-0.90, lowPass * modulation + noise)))
            }
        }
    }
