...
flowchart LR
    ...
    DSP["Bluetooth codec + headset DSP"]
...
- Results from two headset models cannot be assumed comparable.
...
sinusoid, but it does not guarantee a flat headset frequency response, a pure
...
listener's eardrum. Frequency-dependent playback, room leakage, and headset
...
the listener's language knowledge, attention, and recent experience. SNR Lab
exposes three behavioral contrasts related to that interaction.

### Semantic constraint

A strongly constraining sentence makes its final word more predictable than a
neutral sentence. Comparing final-word accuracy across those contexts estimates a
within-run **context benefit**. This does not uniquely measure prediction: lexical
frequency, sentence naturalness, memory, and item difficulty can all contribute.

### Auditory restoration

In the interrupted-speech comparison, the same proportion of waveform is removed
periodically. In one condition it becomes digital silence; in the other it is
replaced by noise. The replacement does not contain the deleted speech. Higher
recognition in the noise-filled condition is consistent with a restoration
benefit, but the app cannot determine the perceptual or neural mechanism from a
typed response alone.

The implementation uses a periodic gate
...
0, & \text{otherwise},
\end{cases}
\qquad T=1/3\ \mathrm{s},
$$

with short ramps at each boundary. For noise-filled gaps, the zero region is
replaced by locally generated random noise. The practical implementation applies
the ramps directly to the PCM buffer.

### Short-term adaptation

A stable transformation can become easier to understand with exposure. The app
compares mean word accuracy for the late and early halves of an eight-item block.
The filter is deliberately simple and reproducible in structure: a first-order
3.2 kHz low-pass response, shallow 3.2 Hz envelope modulation, and additive
low-level noise. Random noise samples differ between presentations.

An early-to-late change can also reflect item difficulty, fatigue, strategy,
practice with the response interface, or regression to the mean. It should not be
called neural plasticity without a controlled study.

These contrasts are motivated by classic primary studies of controlled sentence
predictability, phonemic restoration, and learning of degraded speech. The exact
references and implementation boundaries are in
[`PREDICTIVE_LISTENING.md`](PREDICTIVE_LISTENING.md).

## 12. Live SNR estimator
...
## 13. Compensation-filter physics
