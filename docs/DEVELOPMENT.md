...
- Physical iPhone for Bluetooth/headset routing tests.
...
1. Open `AuditoryInferenceLab.xcodeproj` in Xcode.
2. Select the **AuditoryInferenceLab** target.
3. In **Signing & Capabilities**, enable automatic signing and select a team.
4. The separate app is configured as `com.gjenaro.AuditoryInferenceLab`; replace it
   only if you control a different permanent bundle identifier.
...
```bash
mkdir -p /tmp/auditory-inference-swift-cache /tmp/auditory-inference-clang-cache
env \
  SWIFT_MODULECACHE_PATH=/tmp/auditory-inference-swift-cache \
  CLANG_MODULE_CACHE_PATH=/tmp/auditory-inference-clang-cache \
  xcrun --sdk iphoneos swiftc \
    -typecheck \
    -target arm64-apple-ios17.0 \
    -module-name AuditoryInferenceLab \
    -parse-as-library \
    -module-cache-path /tmp/auditory-inference-swift-cache \
...
The deterministic inference check can be compiled and run on macOS:

```bash
xcrun swiftc \
  SNRLab/Models/Models.swift \
  SNRLab/Models/PredictiveModels.swift \
  SNRLab/Models/InferenceModels.swift \
  Checks/InferenceLogicCheck.swift \
  -module-cache-path /tmp/auditory-inference-check-cache \
  -o /tmp/auditory-inference-logic-check
/tmp/auditory-inference-logic-check
```

It checks bounded profile/filter values, a zero/uncertainty-free Original path,
the six-pair recommendation boundary, and Codable experiment round-trip.
...
of the numerical logic. Do not use it as evidence for headset routing, microphone
behavior, output amplitude, or physical-device timing.

### Physical iPhone without a supported headset

Appropriate for microphone and general audio tests. The bilateral pure-tone setup
should remain gated because it requires a supported AirPods or Bose
QuietComfort-named stereo route.

### Physical iPhone with a supported headset
...
- all four Inference Lab candidates, visible uncertainty, equal-headroom A/B
  switching, randomized label mapping, no-difference recording, persistence, and
  recommendation boundaries;
- all four Predictive Listening blocks, English/Spanish input, replays, saved raw
  responses, summary metrics, and history restoration after relaunch;
- non-repetition of ordinary speech material across consecutive speech, volume,
  and noise-profile runs.
...
- Increment `PredictiveListeningTest.protocolVersion` whenever predictive stimuli,
  allocation, signal processing, scoring, or interpretation changes.
