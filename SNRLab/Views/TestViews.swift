...
struct TestHubView: View {
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                ...
                NavigationLink(destination: ComprehensiveAssessmentView()) {
                    ZStack(alignment: .topTrailing) {
                        RoundedRectangle(cornerRadius: 24)
                            .fill(
                                LinearGradient(
                                    colors: [Color.indigo.opacity(0.34), Color.cyan.opacity(0.18)],
                        ...
                        VStack(alignment: .leading, spacing: 12) {
                            HStack {
                                Label(language.text("GUIDED PROFILE", "PERFIL GUIADO"), systemImage: "point.3.connected.trianglepath.dotted")
                                    .font(.caption.bold()).foregroundStyle(.cyan)
                                Spacer()
                                Image(systemName: "arrow.up.right.circle.fill")
                                    .font(.title2).foregroundStyle(.white)
                            }
                            Text(language.text("Complete Listening Profile", "Perfil auditivo completo"))
                                .font(.title2.bold()).foregroundStyle(.white)
                            Text(language.text(
                                "Follow tones → speech in noise → predictive listening, with saved progress between stages.",
                                "Sigue tonos → habla con ruido → escucha predictiva, guardando el progreso entre etapas."
                            ))
                            .font(.subheadline).foregroundStyle(.white.opacity(0.78))
                            HStack(spacing: 7) {
                                ForEach(0..<3, id: \.self) { index in
                                    Capsule()
                                        .fill(index < completedCoreStages ? Color.cyan : Color.white.opacity(0.18))
                                        .frame(height: 6)
                                }
                            }
                            Text(language.text(
                                "\(completedCoreStages) of 3 stages saved",
                                "\(completedCoreStages) de 3 etapas guardadas"
                            ))
                    ...
                    .overlay(RoundedRectangle(cornerRadius: 24).stroke(Color.cyan.opacity(0.24)))
                }
                .buttonStyle(.plain)
                ...
                TestLink(
                    ...
                    subtitle: language.text(
                        "Find SNR50, SNR80 and SNR90 using a 144-sentence nonrepeating corpus.",
                        "Calcula SNR50, SNR80 y SNR90 con un corpus de 144 frases sin repetición."
                    ),
                ...
                TestLink(
                    ...
                    subtitle: language.text(
                        ...
                        "Contexto, cierre auditivo, costo de predicción, adaptación y esfuerzo auditivo."
                    ),
                    symbol: TestKind.predictiveListening.symbol,
                    tint: .indigo,
                    destination: PredictiveListeningView()
                )
                TestLink(
                ...
                TestLink(
                    ...
                    subtitle: language.text(
                        "Adaptive left/right screening for stereo AirPods or Bose QuietComfort.",
                        "Evaluación adaptativa izquierda/derecha con AirPods o Bose QuietComfort estéreo."
                    ),
                ...
                GlassCard {
                    Text(language.text(
                        "For best repeatability, use the same supported headset, fit, room, and iPhone volume for every session.",
                        "Para comparar resultados, usa los mismos auriculares compatibles, ajuste, lugar y volumen del iPhone en cada sesión."
                    ))
    ...
    private var completedCoreStages: Int {
        (model.profile.latestPureToneTest == nil ? 0 : 1) +
        (model.profile.speechTestPoints.count < 12 ? 0 : 1) +
        (model.profile.latestPredictiveTest == nil ? 0 : 1)
    }
}
...
struct ComprehensiveAssessmentView: View {
    ...
    private var language: TestLanguage { model.selectedLanguage }
    private var toneComplete: Bool { model.profile.latestPureToneTest != nil }
    private var speechComplete: Bool { model.profile.speechTestPoints.count >= 12 }
    private var predictiveComplete: Bool { model.profile.latestPredictiveTest != nil }
    private var completedCount: Int {
        [toneComplete, speechComplete, predictiveComplete].filter { $0 }.count
    }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 16) {
                titleBlock(
                    language.text("Complete Listening Profile", "Perfil auditivo completo"),
                    subtitle: language.text(
                        "A guided end-to-end view of audibility, speech understanding, and predictive listening.",
                        "Una guía integral de audibilidad, comprensión del habla y escucha predictiva."
                    )
                )
                ...
                GlassCard {
                    VStack(alignment: .leading, spacing: 12) {
                        HStack(alignment: .firstTextBaseline) {
                            Text(language.text("Profile progress", "Progreso del perfil")).font(.headline)
                            Spacer()
                            Text("\(completedCount) / 3").font(.title3.bold()).foregroundStyle(.cyan)
                        }
                        ProgressView(value: Double(completedCount), total: 3)
                            .tint(.cyan).scaleEffect(x: 1, y: 1.45)
                        Text(language.text(
                            "Each stage saves independently. You can leave, return, repeat a stage, or open any individual test from the Tests screen.",
                            "Cada etapa se guarda por separado. Puedes salir, volver, repetir una etapa o abrir cualquier prueba individual desde Pruebas."
                        ))
                        .font(.footnote).foregroundStyle(.secondary)
                    }
                }
                ...
                AssessmentStageCard(
                    number: 1,
                    title: language.text("Bilateral pure tones", "Tonos puros bilaterales"),
                    subtitle: language.text(
                        "Measure separate relative thresholds for the left and right ear across eight frequencies.",
                        "Mide umbrales relativos separados para cada oído en ocho frecuencias."
                    ),
                    output: language.text("Relative audiogram + reliability", "Audiograma relativo + fiabilidad"),
                    symbol: "ear.badge.waveform",
                    tint: .purple,
                    complete: toneComplete,
                    language: language,
                    destination: AnyView(FrequencySensitivityView())
                )
                ...
                AssessmentStageCard(
                    ...
                    subtitle: language.text(
                        "Estimate the speech-to-noise ratios associated with 50%, 80%, and 90% recognition.",
                        "Estima las relaciones señal-ruido asociadas con 50%, 80% y 90% de reconocimiento."
                    ),
                    output: "SNR50 · SNR80 · SNR90",
                    symbol: "person.wave.2.fill",
                    tint: .green,
                    complete: speechComplete,
                    language: language,
                    destination: AnyView(SpeechInNoiseTestView())
                )
                ...
                AssessmentStageCard(
                    ...
                    subtitle: language.text(
                        "Compare context, auditory closure, misleading expectations, and adaptation at an individualized difficulty.",
                        "Compara contexto, cierre auditivo, expectativas engañosas y adaptación con dificultad individualizada."
                    ),
                    output: language.text("Four behavioral contrasts + effort", "Cuatro contrastes conductuales + esfuerzo"),
                    symbol: "brain.head.profile",
                    tint: .indigo,
                    complete: predictiveComplete,
                    language: language,
                    destination: AnyView(PredictiveListeningView())
                )

                if completedCount == 3 {
                    NavigationLink(destination: ResultsView()) {
                        Label(language.text("View complete results", "Ver resultados completos"), systemImage: "chart.xyaxis.line")
                ...
                GlassCard {
                    HStack(alignment: .top, spacing: 11) {
                        ...
                        Text(language.text(
                            "This combines complementary screening and behavioral modules; it is not one validated clinical battery and does not diagnose hearing, cognition, or brain function.",
                            "Esto combina módulos complementarios de evaluación y conducta; no es una batería clínica validada ni diagnostica audición, cognición o función cerebral."
                        ))
                        .font(.footnote).foregroundStyle(.secondary)
                    }
                }
        ...
        .background(Color.black)
        .navigationTitle(language.text("Complete Profile", "Perfil completo"))
        .navigationBarTitleDisplayMode(.inline)
    }

    @ViewBuilder
    private var nextStageButton: some View {
        if !toneComplete {
            nextLink(
                title: language.text("Begin with bilateral tones", "Comenzar con tonos bilaterales"),
                destination: FrequencySensitivityView()
            )
        } else if !speechComplete {
            nextLink(
                title: language.text("Continue with speech in noise", "Continuar con habla con ruido"),
                destination: SpeechInNoiseTestView()
            )
        } else {
            nextLink(
                title: language.text("Finish with predictive listening", "Finalizar con escucha predictiva"),
                destination: PredictiveListeningView()
            )
        }
    }

    private func nextLink<Destination: View>(title: String, destination: Destination) -> some View {
...
private struct AssessmentStageCard: View {
    let number: Int
    let title: String
    let subtitle: String
    let output: String
    ...
    var body: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: 13) {
                HStack(spacing: 13) {
                    ZStack {
                        RoundedRectangle(cornerRadius: 15).fill(tint.opacity(0.14)).frame(width: 52, height: 52)
                        Image(systemName: complete ? "checkmark" : symbol).font(.title2.bold()).foregroundStyle(complete ? .green : tint)
                    }
                    VStack(alignment: .leading, spacing: 3) {
                        Text(language.text("STAGE \(number)", "ETAPA \(number)"))
                            .font(.caption2.bold()).foregroundStyle(tint)
                        Text(title).font(.headline)
                    }
                    Spacer()
                    Text(complete ? language.text("SAVED", "GUARDADO") : language.text("READY", "LISTO"))
                        .font(.caption2.bold())
                        .foregroundStyle(complete ? .green : .secondary)
                        .padding(.horizontal, 9).padding(.vertical, 6)
                        .background(Capsule().fill((complete ? Color.green : Color.secondary).opacity(0.12)))
                }
                Text(subtitle).font(.subheadline).foregroundStyle(.secondary)
                Label(output, systemImage: "chart.bar.doc.horizontal")
                    .font(.caption).foregroundStyle(tint)
                NavigationLink(destination: destination) {
                    Label(
                        complete ? language.text("Repeat stage", "Repetir etapa") : language.text("Start stage", "Iniciar etapa"),
                        systemImage: complete ? "arrow.clockwise" : "play.fill"
                    )
                    .frame(maxWidth: .infinity)
                }
                .buttonStyle(.bordered)
...
struct SpeechInNoiseTestView: View {
    ...
    @State private var sentences: [TestSentence] = []
    private let totalTrials = 14

    private var language: TestLanguage { model.selectedLanguage }
    private var sentence: TestSentence {
        if !sentences.isEmpty { return sentences[min(trial, sentences.count - 1)] }
        return TestSentence.bank(for: language)[trial % TestSentence.bank(for: language).count]
    }
    ...
    var body: some View {
        ...
        .navigationBarTitleDisplayMode(.inline)
        .onAppear {
            setDefaultNameIfNeeded(for: .speechInNoise)
            prepareSentences()
        }
    ...
    private func prepareSentences() {
        sentences = model.allocateSentences(language: language, count: totalTrials)
    }
}
...
struct VolumeSensitivityView: View {
    ...
    private var language: TestLanguage { model.selectedLanguage }
    private var sentence: TestSentence {
        if !sentences.isEmpty { return sentences[min(index, sentences.count - 1)] }
        return TestSentence.bank(for: language)[index % TestSentence.bank(for: language).count]
    }
    ...
    private func prepareSentences() {
        sentences = model.allocateSentences(language: language, count: levels.count)
    }
}
...
struct FrequencySensitivityView: View {
    ...
    @State private var headsetFitConfirmed = false
    ...
    private var setupReady: Bool {
        ...
        headsetFitConfirmed && transparencyDisabled && consistentModeConfirmed &&
    ...
    private var limitationCard: some View {
        GlassCard {
            VStack(alignment: .leading, spacing: 8) {
                ...
                Text(language.text(
                    "This test is not a clinical audiogram and cannot diagnose hearing loss. Results depend on headset model, fit, device output, ambient noise, and calibration.",
                    "Esta prueba no es un audiograma clínico y no puede diagnosticar pérdida auditiva. Los resultados dependen del modelo y ajuste de los auriculares, la salida del dispositivo, el ruido ambiental y la calibración."
                ))
                .font(.footnote).foregroundStyle(.secondary)
            }
        }
    ...
    private var setupContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            GlassCard {
                VStack(alignment: .leading, spacing: 12) {
                    Label(language.text("Headset and room check", "Revisión de auriculares y sala"), systemImage: "headphones")
                        .font(.headline)
                    ...
                    Button { refreshRoute() } label: {
                        Label(language.text("Refresh headset status", "Actualizar estado de auriculares"), systemImage: "arrow.clockwise")
                    }
                    .buttonStyle(.bordered)
                    ...
                    setupToggle(
                        language.text("Both sides of the headset are seated securely with a consistent fit.", "Ambos lados de los auriculares están bien colocados con un ajuste uniforme."),
                        isOn: $headsetFitConfirmed
                    )
                    setupToggle(
                        language.text("Transparency or Aware mode is off.", "El modo Transparencia o Consciente está desactivado."),
            ...
            GlassCard {
                VStack(alignment: .leading, spacing: 12) {
                    Text(language.text("Left/right channel check", "Revisión de canales izquierdo/derecho"))
                        .font(.headline)
                    Text(language.text(
                        "Play each check and confirm the sound came only from the named side.",
                        "Reproduce cada comprobación y confirma que el sonido vino solo del lado indicado."
                    ))
                    .font(.footnote).foregroundStyle(.secondary)
    ...
    private var testingContent: some View {
        VStack(alignment: .leading, spacing: 16) {
            ...
            if let routeInterruptedMessage {
                GlassCard {
                    VStack(alignment: .leading, spacing: 10) {
                        Label(language.text("Test paused", "Prueba en pausa"), systemImage: "headphones")
                            .font(.headline).foregroundStyle(.orange)
    ...
    private func refreshRoute() {
        do {
            ...
            routeMessage = status.isReadyForBilateralTest ? nil : language.text(
                "Connect stereo AirPods or Bose QuietComfort headphones. iOS reports the route; confirm each physical side with the channel check.",
                "Conecta AirPods o auriculares Bose QuietComfort estéreo. iOS informa la ruta; confirma cada lado físico con la prueba de canales."
            )
        } catch {
        ...
        do {
            ...
            guard status.isReadyForBilateralTest else {
                routeInterruptedMessage = language.text(
                    "The supported stereo headset is no longer available. Reconnect it without changing the test volume.",
                    "Los auriculares estéreo compatibles ya no están disponibles. Reconéctalos sin cambiar el volumen de prueba."
                )
                return
            }
            routeStatus = status
    ...
    private func finalizeAndSave() {
        ...
        let test = BilateralPureToneTest(
            ...
            outputRouteName: routeStatus?.name ?? "Supported stereo headset",
            outputChannelCount: routeStatus?.channelCount ?? 2,
            airPodsSetupConfirmed: setupReady && routeStatus?.isAirPods == true,
            supportedHeadsetSetupConfirmed: setupReady
        )
...
struct NoiseProfilesView: View {
    ...
    var body: some View {
        ...
        .onChange(of: model.selectedLanguage) { _, _ in
            if trial == 0 && !finishedNoise {
                testName = model.defaultTestName(for: .noiseProfile, language: language)
                prepareSentences()
            }
        }
        .onDisappear { audio.stop() }
