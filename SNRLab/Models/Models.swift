...
enum SupportedHeadsetClassifier {
    static func isAirPodsRouteName(_ name: String) -> Bool {
        name.lowercased().contains("airpods")
    }

    /// Accepts common iOS route labels such as "Bose QuietComfort Ultra",
    /// "Bose QC45", "Bose QC 35 II", and user-prefixed variants.
    static func isBoseQuietComfortRouteName(_ name: String) -> Bool {
        let normalized = name.lowercased()
        let tokens = normalized
            .components(separatedBy: CharacterSet.alphanumerics.inverted)
            .filter { !$0.isEmpty }
        guard tokens.contains("bose") else { return false }

        let hasQuietComfortName = normalized.contains("quietcomfort") ||
            normalized.contains("quiet comfort")
        let hasQCModelToken = tokens.contains { token in
            guard token.hasPrefix("qc") else { return false }
            let suffix = token.dropFirst(2)
            return suffix.isEmpty || suffix.allSatisfy(\.isNumber)
        }
        return hasQuietComfortName || hasQCModelToken
    }
}
...
struct BilateralPureToneTest: Identifiable, Codable, Hashable {
    ...
    /// Legacy field retained so existing saved AirPods tests remain decodable.
    var airPodsSetupConfirmed: Bool
    var supportedHeadsetSetupConfirmed: Bool?
    ...
    init(
        ...
        supportedHeadsetSetupConfirmed: Bool? = nil,
        ...
        self.supportedHeadsetSetupConfirmed = supportedHeadsetSetupConfirmed ?? airPodsSetupConfirmed
...
enum TestKind: String, CaseIterable, Codable, Identifiable {
    ...
    func displayName(in language: TestLanguage) -> String {
        ...
        case .predictiveListening: return language.text("Predictive Listening", "Escucha predictiva")
        }
    }
    ...
    var symbol: String {
        ...
        case .predictiveListening: return "brain.head.profile"
        }
    }
}
...
struct TestSentence: Identifiable, Hashable {
    ...
    /// A 12 × 12 matrix produces 144 complete, grammatically compatible
    /// sentences per language. AppModel dispenses them without replacement and
    /// only starts a new cycle after every sentence in the language has been used.
    static let englishBank: [TestSentence] = makeBank(
        subjects: [
            "The careful teacher", "The friendly neighbor", "The local doctor",
            "The patient driver", "The young musician", "The skilled baker",
            "The quiet student", "The busy gardener", "The helpful nurse",
            "The curious visitor", "The early traveler", "The thoughtful artist"
        ],
        actions: [
            "placed the blue folder beside the window.",
            "carried a small package into the office.",
            "checked the morning schedule before breakfast.",
            "found the missing key under the table.",
            "opened the wooden gate near the garden.",
            "brought fresh fruit to the kitchen.",
            "wrote a short message after the meeting.",
            "moved the empty chair across the room.",
            "saved the final document on the computer.",
            "washed the coffee cup after lunch.",
            "followed the narrow path toward the station.",
            "closed the heavy door before the rain."
        ]
    )

    static let spanishBank: [TestSentence] = makeBank(
        subjects: [
            "La maestra cuidadosa", "El vecino amable", "La doctora local",
            "El conductor paciente", "La joven música", "El panadero experto",
            "El estudiante tranquilo", "La jardinera ocupada", "El enfermero atento",
            "La visitante curiosa", "El viajero madrugador", "La artista reflexiva"
        ],
        actions: [
            "puso la carpeta azul junto a la ventana.",
            "llevó un paquete pequeño a la oficina.",
            "revisó el horario de la mañana antes del desayuno.",
            "encontró la llave perdida debajo de la mesa.",
            "abrió la puerta de madera cerca del jardín.",
            "trajo fruta fresca a la cocina.",
            "escribió un mensaje corto después de la reunión.",
            "movió la silla vacía al otro lado del cuarto.",
            "guardó el documento final en la computadora.",
            "lavó la taza de café después del almuerzo.",
            "siguió el camino estrecho hacia la estación.",
            "cerró la puerta pesada antes de la lluvia."
        ]
    )

    private static func makeBank(subjects: [String], actions: [String]) -> [TestSentence] {
        subjects.flatMap { subject in
            actions.map { action in TestSentence(text: "\(subject) \(action)") }
        }
    }
}
