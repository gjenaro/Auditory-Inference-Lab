...
struct RootView: View {
    var body: some View {
        TabView {
            ...
            NavigationStack { InferenceLabView() }
                .tabItem { Label("Lab", systemImage: "brain.head.profile") }
    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 18) {
                titleBlock(
                    model.selectedLanguage.text("Auditory Inference Lab", "Laboratorio de Inferencia Auditiva"),
                    subtitle: model.selectedLanguage.text(
                        "Measure hearing, speech in noise, and predictive listening—then test personalized audio candidates blindly.",
                        "Mide audición, habla con ruido y escucha predictiva; después prueba candidatos de audio personalizado a ciegas."
                    )
                )

                NavigationLink(destination: InferenceLabView()) {
                    GlassCard {
                        HStack(spacing: 16) {
                            ZStack {
                                RoundedRectangle(cornerRadius: 16).fill(.indigo.opacity(0.16)).frame(width: 58, height: 58)
                                Image(systemName: "brain.head.profile").font(.title).foregroundStyle(.indigo)
                            }
                            VStack(alignment: .leading, spacing: 5) {
                                Text(model.selectedLanguage.text("Personalization Lab", "Laboratorio de personalización"))
                                    .font(.title3.bold())
                                Text(model.selectedLanguage.text(
                                    "Inspect uncertainty, compare filter candidates, and record blinded preferences.",
                                    "Revisa la incertidumbre, compara filtros y registra preferencias a ciegas."
                                ))
                    ...
                    GlassCard {
                        HStack(spacing: 16) {
                            ...
                            VStack(alignment: .leading, spacing: 5) {
                                ...
                                Text("Pure tones, speech in noise, predictive listening, volume, and noise profiles.").foregroundStyle(.secondary)
                            }
                            Spacer()
                            Image(systemName: "chevron.right").foregroundStyle(.secondary)
                        }
...
struct ResultsView: View {
    ...
    private var availableReliabilityScores: [Double] {
        [pureToneTest?.reliabilityScore, predictiveTest?.reliabilityScore].compactMap { $0 }
    }
    private var overallConfidence: Double? {
        guard !availableReliabilityScores.isEmpty else { return nil }
        return availableReliabilityScores.reduce(0, +) / Double(availableReliabilityScores.count)
    }
            ...
            VStack(alignment: .leading, spacing: 16) {
                ...
                if let predictiveTest {
                    ...
                    GlassCard {
                        VStack(spacing: 12) {
                            ContentUnavailableView(
                                language.text("No predictive profile yet", "Aún no hay perfil predictivo"),
                                systemImage: "brain.head.profile",
                                description: Text(language.text(
                                    "Measure context benefit, auditory closure, prediction cost, adaptation, and listening effort.",
                                    "Mide beneficio del contexto, cierre auditivo, costo de predicción, adaptación y esfuerzo auditivo."
                                ))
                            )
                            NavigationLink(destination: PredictiveListeningView()) {
                                Label(language.text("Run Predictive Listening", "Iniciar Escucha predictiva"), systemImage: "play.fill")
                                    .frame(maxWidth: .infinity)
                            }
                            .buttonStyle(.borderedProminent)
                            .tint(.indigo)
                    ...
                    if pureToneTest != nil || predictiveTest != nil || !model.profile.speechTestPoints.isEmpty {
                        VStack(alignment: .leading, spacing: 11) {
                            if let pureToneTest {
                                ...
                                qualityRow(
                                    language.text("Pure-tone reliability", "Fiabilidad de tonos puros"),
                            ...
                            if let predictiveTest {
                                qualityRow(
                                    ...
                                    value: "\(predictiveTest.trials.count)"
                                )
                                qualityRow(
                                    language.text("Predictive protocol reliability", "Fiabilidad del protocolo predictivo"),
                                    value: "\(Int(predictiveTest.reliabilityScore.rounded()))%"
                                )
                            }
                            if let overallConfidence {
                                ...
                                HStack {
                                    VStack(alignment: .leading, spacing: 2) {
                                        Text(language.text("Available-protocol confidence", "Confianza de protocolos disponibles"))
                                            .font(.headline)
                                        Text(language.text(
                                            "Mean of available heuristic reliability scores",
                                            "Media de las puntuaciones heurísticas disponibles"
                                        ))
                                        .font(.caption).foregroundStyle(.secondary)
                                    }
                                    Spacer()
                                    Text("\(Int(overallConfidence.rounded()))%")
                                        .font(.title2.bold())
                                        .foregroundStyle(overallConfidence < 75 ? .orange : .green)
                                }
                            }
                        }
                    } else {
                        ContentUnavailableView(
                            language.text("No test-quality data", "No hay datos de calidad"),
...
struct TestRecordDetailView: View {
    ...
    private var detailContent: some View {
        ...
        case .predictiveListening:
            if let test = record.predictiveTest {
                PredictiveListeningSummary(test: test, language: model.selectedLanguage)
            } else {
                ContentUnavailableView(
                    model.selectedLanguage.text("Predictive record unavailable", "Registro predictivo no disponible"),
                    systemImage: "brain.head.profile"
                )
            }
        }
    ...
    var body: some View {
        Form {
            ...
            Section("Audio") {
                ...
                Text("For pure-tone and relative-volume tests, choose a comfortable iPhone volume and do not change it during the test. Pure-tone testing requires supported stereo AirPods or Bose QuietComfort headphones and tests each ear separately.")
                    .font(.footnote).foregroundStyle(.secondary)
            }
            Section("Measurement limits") {
                Text("Live SNR uses an adaptive microphone estimate. Volume and pure-tone tests use relative digital levels, not calibrated dB SPL or dB HL. Headset processing, fit, system volume, ambient noise, and device model can change the result.")
                    .font(.footnote).foregroundStyle(.secondary)
            }
            Section("Safety") {
